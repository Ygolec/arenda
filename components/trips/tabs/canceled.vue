        ...
        <template v-for="column in columns" :key="column.key">
          <th>
            ...
            <template v-if="column.value === 'tools'">
              <v-btn variant="text" @click="fetchCanceledReservations()" icon="refresh"></v-btn>
            </template>
          </th>
        </template>
      </tr>
...
const fetchCanceledReservations = async () => {
  try {
    canceledReservations.value = await $fetch('/api/reservations/canceled-bookings', {
      body: {token: token.value},
      method: 'POST'
    },)
  } catch (e) {
    console.error('Ошибка при получении отмененных бронирований:', e);
  } finally {
  }
};
